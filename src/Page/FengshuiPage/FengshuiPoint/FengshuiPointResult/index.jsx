...
const FengshuiPointResult = () => {
  ...
  useEffect(() => {
    if (element.elementId) {
      fetch(
        `https://localhost:7275/api/Advertisement/GetRecAds?Elementid=${element.elementId}`
      )
  ...
  const totalKoiPoint = koiPoint.reduce(
    (acc, koi) => acc + koi.patternPoint,
    0
  );
  const progressWidth = (totalKoiPoint / (koiPoint.length * 10)) * 100;

  // Tính toán các loại màu sắc dựa trên giá trị
  const sortedColors = element.elementColors
    ? element.elementColors.sort((a, b) => b.values - a.values)
    : [];
  const maxValue = sortedColors.length > 0 ? sortedColors[0].values : 0;
  const minValue =
    sortedColors.length > 0 ? sortedColors[sortedColors.length - 1].values : 0;

  const t_sinh = sortedColors.filter((color) => color.values === maxValue);
  const hoa_hop = sortedColors.filter(
    (color) => color.values < maxValue && color.values > 0
  );
  const che_khac = sortedColors.filter(
    (color) => color.values < 0 && color.values >= -2
  );
      ...
      <div className="max-w-5xl mx-auto p-6 bg-white rounded-lg shadow-md">
        ...
        <div className="mb-6 flex items-center">
          <p className="font-semibold mr-2">
            ...
            <span
              className={`text-${
                elementColorsC[element.element1] || "text-gray-600"
              }`}
            >
              {" "}
              {element.element1 || "Chưa xác định"}
            </span>
          </p>
          <div className="w-8 h-8 rounded-full">
            <img
              src={elementColorsP[element.element1] || "default_image_url"}
              alt={element.element1 || "Mệnh không xác định"}
            />
          </div>
        </div>
          ...
          <div className="h-4 bg-gray-200 rounded-full">
            <div
              className={`h-full rounded-full ${
                elementColorsT[element.element1] || "blue-500"
              }`}
        ...
        <p className="mb-6">
          Hướng đặt bể cá bạn chọn là:{" "}
          <span className="font-semibold">
            {direction} (Điểm: {bonusDirection}/1)
          </span>
        </p>
        <p className="mb-6">
        ...
        <p className="mb-6">
          Tổng số lượng cá bạn chọn là:{" "}
          <span className="font-semibold">
            {totalAmount} (Điểm: {bonusQuantity}/1)
          </span>
        </p>
                  ...
                  <div className="flex">
                    <div className="w-[100px] h-[170px] flex-shrink-0">
                      <img
                        ...
                        onClick={() =>
                          handleImageClick(koi.imageUrl || "default_image_url")
                        }
                      />
                    </div>
          ...
          <div className="w-1/3 pl-4">
            <div className="bg-gray-100 p-4 rounded-lg h-full">
              <p className="text-xl font-semibold mb-2">
                Tổng điểm: {totalPoint.toFixed(1)} / 10
              </p>
          ...
          <div className="grid grid-cols-4 gap-4">
            <div>
              ...
              <table className="min-w-full border-collapse border border-gray-200">
                <thead
                  className={`${
                    elementColorsT[element.element1] || "gray-200"
                  } text-white`}
                >
                  <tr>
