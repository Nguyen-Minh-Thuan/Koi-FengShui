...
const UserAdsManageDetail = ({ ad, onClose }) => {
  ...
  const packageName =
    packages.find((pkg) => pkg.packageId === ad.packageId)?.packageName ||
    "Chưa chọn gói";

  const totalPrice =
    ad.transactions && ad.transactions.length > 0
  ...
  const statusLabel =
    ad.status.status1 === "Drafted"
      ...
      ? "Đang quảng cáo"
      : ad.status.status1 === "Pending"
      ? "Chờ duyệt"
      : ad.status.status1 === "Sold Out"
      ? "Đã bán"
      : ad.status.status1 === "Expried"
      ? "Hết hạn"
      : "Không xác định";
  ...
  const handleDeleteAd = (adId, userId, onClose) => {
    ...
    fetch(
      `https://localhost:7275/api/Advertisement/DeleteDraftById?id=${ad.adsId}&userId=${userId}`,
      {
        method: "DELETE",
        headers: {
  ...
  const handleMarkAsSold = (adsId, userId) => {
    ...
    fetch(
      `https://localhost:7275/api/Advertisement/AdsSoldOut?id=${adsId}&userId=${userId}`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  return (
    ...
    >
      <div
        className="bg-white p-4 rounded-lg max-w-5xl mx-auto h-[500px] overflow-y-auto relative"
        ...
        <div className="max-w-5xl mx-auto p-4 font-sans ">
          <div className="flex flex-col md:flex-row gap-8">
            <img
              src={ad.imageUrl}
              alt={ad.title}
              className="w-full md:w-[233px] h-[350px] object-contain"
            />
          ...
          <div className="text-gray-800">
            ...
            <ul className="list-disc list-inside">
              ...
              <li>
                Ngày đăng:{" "}
                <strong>
                  {ad.startedDate ? formatDate(ad.startedDate) : "Chưa bắt đầu"}
                </strong>
              </li>
              <li>
                Ngày hết hạn:{" "}
                <strong>
                  {ad.expiredDate ? formatDate(ad.expiredDate) : "Chưa bắt đầu"}
                </strong>
              </li>
              <li>
                Gói đăng ký: <strong>{packageName}</strong>
              </li>
              <li>
                Số ngày đăng:{" "}
                <strong>
                  {ad.duration ? `${ad.duration} ngày` : "Chưa xác định"}
                </strong>
              </li>
              <li>
                Tổng tiền đã trả: <strong>{totalPrice}</strong>
              </li>
            </ul>
          </div>
      ...
      {showPackagePopup && (
        ...
        >
          <div
            className="bg-white p-4 rounded-lg max-w-5xl mx-auto h-[570px] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <ChoosePackagePage
              adsId={ad.adsId}
